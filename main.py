...
# cron job for fetching data from Copernicus's database
@app.route('/tasks/getsla')
def getsla():
    window = 1 
    # today's and yesterday's date
    today = (datetime.datetime.now()).strftime('%Y-%m-%d')
    yesterday = (datetime.datetime.now() - timedelta(window)).strftime('%Y-%m-%d')

    # fetching some data from copernicus 
    bpos = "motuclient.py -f bpos.nc -m http://nrt.cmems-du.eu/motu-web/Motu -s SEALEVEL_GLO_PHY_L4_NRT_OBSERVATIONS_008_046-TDS -d dataset-duacs-nrt-global-merged-allsat-phy-l4 -x 87 -X 89 -y 11 -Y 13 -v sla -o /tmp -u jbootkrajang -p $429seL83  -t " + yesterday + " -T " + today
    bneg = "motuclient.py -f bneg.nc -m http://nrt.cmems-du.eu/motu-web/Motu -s SEALEVEL_GLO_PHY_L4_NRT_OBSERVATIONS_008_046-TDS -d dataset-duacs-nrt-global-merged-allsat-phy-l4 -x 96 -X 98 -y 11 -Y 13 -v sla -o /tmp -u jbootkrajang -p $429seL83  -t " + yesterday + " -T " + today
    spos = "motuclient.py -f spos.nc -m http://nrt.cmems-du.eu/motu-web/Motu -s SEALEVEL_GLO_PHY_L4_NRT_OBSERVATIONS_008_046-TDS -d dataset-duacs-nrt-global-merged-allsat-phy-l4 -x 94 -X 96 -y -2 -Y 2 -v sla -o /tmp -u jbootkrajang -p $429seL83  -t " + yesterday + " -T " + today
    sneg = "motuclient.py -f sneg.nc -m http://nrt.cmems-du.eu/motu-web/Motu -s SEALEVEL_GLO_PHY_L4_NRT_OBSERVATIONS_008_046-TDS -d dataset-duacs-nrt-global-merged-allsat-phy-l4 -x 88 -X 99 -y -10 -Y -8 -v sla -o /tmp -u jbootkrajang -p $429seL83  -t " + yesterday + " -T " + today

    idxList = [bpos, bneg, spos, sneg]

    dset = []
    for idx in idxList:
        sys.argv = idx.split() # populating system arguments
        _options = load_options() # constructing motuclient's option

        # fetching information
        filename = '/tmp/' + sys.argv[2]
        try:
            # try reading from file, if exist
            fin = open(filename)
            fin.close()
        except:
            ...
            dset.append(nc.Dataset(filename))
            

    N = len(dset[0]['sla'])
    for i in range(N):
        # reading information from file and put them in the datastore
        delta = window-i
        if delta >= 0:
            curDate = (datetime.datetime.now() - timedelta(delta)).strftime('%Y-%m-%d')
            slaRec = {'date':curDate,'bpos':np.mean(dset[0]['sla'][i]),
                    'bneg':np.mean(dset[1]['sla'][i]),
                    'spos':np.mean(dset[2]['sla'][i]),
                    'sneg':np.mean(dset[3]['sla'][i])}
            storeSLA(curDate, slaRec)
            print('[info] new SLA added for ' + curDate)

    return "[info] SLA update completed" 

# fetching rain info from TMD
@app.route('/tasks/getrain')
def getrain_entry():
    threading.Thread(target=getrain).start()
    return "[info] RainMM update completed" 

def getrain():
    headers = {'User-Agent': 'Mozilla/5.0'}
    URL = "https://www.tmd.go.th/cis/freedata.php?method=fd_SearchDaily"
    # open station file and looping thru each station 
    metStnFile = 'static/thailand_metstation.csv'
    ...
    for window in range(0):
        today = (datetime.datetime.now() - timedelta(window)).strftime('%-d %-m %Y').split()
        #print(window, today)
        today2format = (datetime.datetime.now() - timedelta(window)).strftime('%Y-%m-%d')
